...
class Settings {
  constructor(config) {
    ...
    this.dom.addEventListener('mousedown', (event) => {
      if((event.target.name == `stopKey` || event.target.name == `fishingKey` || event.target.name == `luresKey` || event.target.name == `poleKey` || event.target.name == `spareKey`) && !event.target.disabled) {
